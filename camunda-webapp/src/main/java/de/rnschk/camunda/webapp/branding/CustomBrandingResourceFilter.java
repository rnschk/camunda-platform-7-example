...
@Component
@ConditionalOnProperty(value = "de.rnschk.camunda.branding.enabled", matchIfMissing = true)
public class CustomBrandingResourceFilter implements Filter {
    ...
    private static void preloadCss(final String buttonColor, final String topColor) throws IOException {
        var customStyles = IOUtils.toString(currentThread().getContextClassLoader().getResource(RESOURCE_PATH_CUSTOM_STYLES), UTF_8);
        cssResource = customStyles.replace(CSS_PLACEHOLDER_BUTTON_COLOR, buttonColor)
